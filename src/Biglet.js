...
class Biglet {
  constructor (owner) {
    ...
    this.catchFn = function (err) {
      debug('[BIGLET domid=' + this.domid + '] : ' + err.message)
  ...
  /**
   * 添加子 biglet
   * @param { constructor } ChildBiglet
   * @param { object } args 传入的参数
   */
  addChild (ChildBiglet, ...args) {

    const childPagelet = new ChildBiglet(...args)
    this.children.push(childPagelet)
  }
  ...
  render () {
    if (this.owner && this.owner.done) {
      debug('[BIGLET WARNING] bigview is alread done, there is no need to render biglet module!')
      return Promise.resolve()
    }
  ...
  get view () {
    return `<script type="text/javascript" charset="utf-8">bigview.view(${this._payload})</script>`
  }
